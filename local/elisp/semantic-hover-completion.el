;;; semantic-hover-completion.el --- Better tooltip completion for Semantic Bovinator

;; Copyright (C) 2007  Free Software Foundation, Inc.

;; Author: Maciej Katafiasz <email@mathrick.org>
...
a list of (POS CANDIDATES ORIG-CANDIDATES CTX). POS is an integer indicating how many
self-inserting keys were entered since the ORIG-CANDIDATES were created. If it's
0, CADIDATES is equal to ORIG-CANDIDATES, otherwise it's a filtered down list with
...
`semantic-hover-completion-popup' immediately before showing the tooltip, to ensure
keys go to the right place."
  (set-keymap-parent semantic-hover-completion-keymap (car (current-active-maps))))

(defun semantic-hover-completion-make-keymap ()
  "Make keymap used by hover completion mode, based on `semantic-hover-completion-keys'"
  (let ((keymap (make-sparse-keymap)))
    (mapcar (lambda (elt)
	      (define-key keymap
		elt (lambda () (interactive)
		      (semantic-hover-completion-do-popup 1))))
	    (cdr (assq 'navigate-candidates-forward semantic-hover-completion-keys)))
    (mapcar (lambda (elt)
	      (define-key keymap
		elt (lambda () (interactive)
		      (semantic-hover-completion-do-popup -1))))
	    (cdr (assq 'navigate-candidates-backward semantic-hover-completion-keys)))
    (mapcar (lambda (elt)
    ...
    ;; By default we want all unspecified keys to dismiss the pop-up and spill over
...
(defvar semantic-hover-completion-keymap (semantic-hover-completion-make-keymap)
  "Keymap to be used with completion tooltip. It needs to be prepared before actual use
to have the correct parent, use `semantic-hover-completion-prepare-keymap' for that.")

(defun semantic-hover-completion-guesstimate-y-offset ()
  "Attempt to guess how much we need to shift the tooltip to get it displayed right
...
(defun semantic-hover-completion-do-popup (n)
  "Do the actual work of showing and navigating the completion pop-up. Integer argument n
specifies how many positions should in the pop-up should the highlight be moved."
(when semantic-hover-completion-active
  (let* ((strings (semantic-hover-completion-get-candidates-names))
    ...
    (setq strings (propertize-nth semantic-hover-completion-active
				  (propertize-all strings 'face 'semantic-hover-completion-face)
				  'face 'semantic-hover-completion-highlight-face))
...
(defun semantic-hover-completion-dismiss ()
  "Dismiss the active completion tooltip, undoing any keymap changes done during pop-up."
  (interactive)
  (remove 'minor-mode-map-alist
	  (cons 'semantic-hover-completion-active semantic-hover-completion-keymap))
  (setq semantic-hover-completion-active nil))

(defun semantic-hover-completion-dismiss-or-filter ()
  "Possibly dismiss the popup and replay the last input event, if it would be something
else than `self-insert-command', otherwise insert the character and filter down candidates.

This function MUST be bound to a default keymap entry (ie, with t as the key), otherwise
it'll fail to recognise which events need replaying and bad things will happen."
  (interactive)
  (let ((was-active semantic-hover-completion-active)
	(semantic-hover-completion-active nil))
    (if (not (eq (key-binding (this-command-keys-vector) t) 'self-insert-command))
	(progn
...
(defun semantic-hover-completion-filter (char)
  "Insert self-inserting keystrokes, filtering down candidates list based on input."
  (when semantic-hover-completion-active
...
(defun semantic-hover-completion-unfilter ()
  "Handle deletion of previously entered filter keystrokes, widening the candidate list.
Possibly try also going before the original starting point, if semantic context indicates
it's possible, though that's a fairly expensive operation requiring recomputing the full
candidates list."
  (interactive)
  ...
  (when semantic-hover-completion-active
    (let ((cands (nth 1 semantic-hover-completion-candidates))
	  ...
	  (let ((pref (substring 
		       ...
		       0 (incf (nth 0 semantic-hover-completion-candidates) -1))))
...
(defun semantic-hover-completion-commit ()
  ...
  (when semantic-hover-completion-active
    (let ((idx semantic-hover-completion-active)
      ...
      (flet ((insert (&rest args) (apply 'insert-and-inherit args)))
	(semantic-ia-insert-tag (nth idx cands)))
      (semantic-hover-completion-dismiss))))

(defmacro semantic-hover-completion-add-electric-string (str)
"Cause the string specified by str be electric, ie. whenever the last character of
it is input in the buffer, and forms the complete string, it will cause
`semantic-hover-completion-popup' to fire. The last character of all electric strings
needs to be unique, otherwise new electric strings will cancel previously defined ones.
Strings are electric locally to the major mode, so this macro's output is a perfect
candidate for running in a mode hook.

Str must contain only ordinary characters, ie. ones for which `self-insert-command' would
normally be defined. It might be a string literal or a symbol, but its value must be defined
at the time this macro is called."
;; Eval all references to str to allow both literals and symbols with string value
  (if (stringp (eval str))
      `(lambda ()
			  ...
			  (let ((succ nil))
				...
				(save-match-data
				  ...
				  (if (= (match-end 0) (point))
				      (setq succ t)))))
			    ;; Actual call is done here to avoid point being returned
